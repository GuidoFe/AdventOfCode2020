...
def main():
    f = open("input")
    # 0 = reading rules for fields, 1 = reading my ticket, 2 = reading other tickets
    status = 0
    validRanges = []
    fieldRanges = {}
    fieldNames = []
    ...
    for l in f:
        ...
        if len(l) > 0:
            ...
            elif (status == 1 or status == 2) and l != "nearby tickets:" and l != "your ticket:":
                values = list(map(lambda x: int(x), l.split(",")))
                if status == 1:
                ...
                if validTicket:
                    for pos, v in enumerate(values):
                        ...
                        if len(fieldPools[pos]) > 1:
                            invalidFields = []
                            # Check for which fields in that position the current value would not be valid
                            ...
                            # Delete the impossible fields from the pool of possibilities
                            for f in invalidFields:
                                fieldPools[pos].remove(f)
                            # If there is only one field left in the pool...
                            if len(fieldPools[pos]) == 1:
                                # ... delete that field from the pool of every other position
